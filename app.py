...
def init_db():
    ...
    if db_type == 'postgres':
        ...
        if not pg_column_exists('email_config', 'unit_price'):
            cur.execute("ALTER TABLE email_config ADD COLUMN unit_price NUMERIC(14,4) NOT NULL DEFAULT 0")
        ...
        # Migration: add global daily SMS limit to team_config
        try:
            cursor = db.execute("PRAGMA table_info(team_config)")
            cols = [row[1] for row in cursor.fetchall()]
        ...
        try:
            ...
            ec_cols = [row[1] for row in db.execute("PRAGMA table_info(email_config)").fetchall()]
            if 'unit_price' not in ec_cols:
                db.execute("ALTER TABLE email_config ADD COLUMN unit_price REAL NOT NULL DEFAULT 0")
...
def _team_limit_value(db, team_admin_id):
    """daily_sms_limit for a team_admin id (0 = sin limite). Returns 0 if none."""
    if not team_admin_id:
        return 0
    row = db.execute(
    ...
    if row and row['daily_sms_limit']:
        return int(row['daily_sms_limit'])
    return 0


def get_global_daily_sms_limit(db):
    """Limite diario global (definido por el administrador del sistema).
    Se guarda en team_config.global_daily_sms_limit (mismo valor en todas las
    filas; se lee el maximo no nulo). 0 = sin limite."""
    row = db.execute(
        "SELECT MAX(global_daily_sms_limit) AS g FROM team_config"
    ).fetchone()
    if row and row['g']:
        return int(row['g'])
    return 0
...
def set_global_daily_sms_limit(db, limit):
    """Persiste el limite global. Si existe al menos una fila de team_config,
    actualiza todas; si no existe ninguna, crea una fila marcadora asociada al
    administrador (no viola la FK) que el listado de equipos ignora."""
    existing = db.execute("SELECT COUNT(*) AS c FROM team_config").fetchone()
    if existing and existing['c']:
        db.execute("UPDATE team_config SET global_daily_sms_limit=?", (limit,))
    else:
        admin_id = g.user['id'] if g.get('user') else 1
        db.execute(
            "INSERT INTO team_config (team_admin_id, daily_sms_limit, global_daily_sms_limit) "
            "VALUES (?, 0, ?)",
            (admin_id, limit)
        )
    db.commit()
...
def effective_daily_sms_limit(db, user):
    """Limite diario efectivo para un miembro: el mas estricto (menor positivo)
    entre el limite de su equipo y el limite global. 0 = sin limite.
    Solo aplica a team_member."""
    limits = []
    # Limite del equipo del miembro
    team_limit = _team_limit_value(db, user.get('team_creator_id'))
    if team_limit > 0:
        limits.append(team_limit)
    # Limite global (definido por el administrador del sistema)
...
def send_sms():
    ...
    if g.user['role'] == 'team_member':
        eff_limit = effective_daily_sms_limit(db, g.user)
        if eff_limit > 0:
            used = count_today_sms(db, g.user['id'])
            if used + len(phones) > eff_limit:
                return jsonify({
                    'error': 'Limite diario alcanzado: ya enviaste %s de %s SMS hoy. Este envio requiere %s numero(s).'
                             % (used, eff_limit, len(phones))
                }), 429
...
def sms_statistics():
    ...
    # ------------------------------------------------------------------
    # The top KPI cards use the SAME provider-reconciliation numbers as the
    # "Conciliacion con operador" panel, so the dashboard presents one
    # consistent set of figures (previously "Tasa de exito" was in-flight/all
    # and "Total Enviados" dropped delivered rows, giving two contradictory
    # rates on the same screen). "Enviados Hoy" = accepted today in the
    # selected report timezone; without a filter it is today, with a filter it
    # is accepted within that range.
    # ------------------------------------------------------------------
    if date_from or date_to or account_filter:
        top_sent = recon.get('submitted', 0)
        today_total = recon.get('total', 0)
    else:
        tday = (datetime.now() + timedelta(hours=rtz_offset)).strftime('%Y-%m-%d')
        today_row = db.execute(
            f"""SELECT
                  COALESCE(SUM(CASE WHEN r.api_code=0 THEN 1 ELSE 0 END),0) AS accepted,
                  COUNT(*) AS total
                FROM sms_records r
        ...
        top_sent = int(today_row['accepted'] or 0)
        today_total = int(today_row['total'] or 0)

    today_sent = top_sent
    total_sent = recon.get('submitted', 0)
    total_pending = recon.get('in_flight', 0)
    total_failed = recon.get('rejected', 0)
    total_all = recon.get('total', 0)
    success_rate = recon.get('delivery_rate', 0.0)

    # 7-day series on the same reconciliation basis (provider accepted =
    # api_code 0, non-simulated), bucketed by report timezone, so the chart
    # matches the Aceptados card. With an explicit range the recon block
    # already returns every day in it; otherwise show the last 7 days.
    _recon_days = {str(x['date']): int(x['submitted'] or 0) for x in recon.get('by_day', [])}
    if date_from and date_to:
        try:
            ...
            if _span > 60:
                _span, _start = 6, datetime.now() - timedelta(days=6)
            last_7_days = [{'date': (_start + timedelta(days=i)).strftime('%Y-%m-%d'), 'count': 0}
                           for i in range(_span + 1)]
        except ValueError:
            last_7_days = []
    elif date_from or date_to:
        _anchor = datetime.strptime(date_to or date_from, '%Y-%m-%d')
        last_7_days = [{'date': (_anchor - timedelta(days=i)).strftime('%Y-%m-%d'), 'count': 0}
                       for i in range(6, -1, -1)]
    else:
        _now_tz = datetime.now() + timedelta(hours=rtz_offset)
        last_7_days = [{'date': (_now_tz - timedelta(days=i)).strftime('%Y-%m-%d'), 'count': 0}
                       for i in range(6, -1, -1)]
    last_7_days = [
        {'date': d['date'], 'count': int(_recon_days.get(d['date'], 0) or 0)}
        for d in last_7_days
    ]

    # Contacts are role-scoped (team_admin: whole team, member: own). Templates
    # are shared across every account, so that total stays global.
    if g.user['role'] == 'team_member':
        total_contacts = db.execute(
            "SELECT COUNT(*) as count FROM contacts WHERE created_by=?", [g.user['id']]
        ).fetchone()['count']
    elif g.user['role'] == 'team_admin':
        total_contacts = db.execute(
            "SELECT COUNT(*) as count FROM contacts WHERE created_by IN "
            "(SELECT id FROM users WHERE id=? OR team_creator_id=?)",
            [g.user['id'], g.user['id']]
        ).fetchone()['count']
    else:
...
def _recon_scope_sql(user):
    """Role-scoped WHERE fragment over sms_records r for reconciliation cards."""
    if user['role'] == 'team_member':
        return ' AND r.created_by = ?'
    if user['role'] == 'team_admin':
        return ' AND (r.created_by IN (SELECT id FROM users WHERE id=? OR team_creator_id=?))'
    return ''


def _recon_scope_params(user):
    if user['role'] == 'team_member':
        return [user['id']]
    if user['role'] == 'team_admin':
...
def get_email_unit_price():
    """Precio unitario global por correo (email_config.unit_price). 0 si no hay."""
    row = get_email_config_row()
    if row and 'unit_price' in row.keys() and row['unit_price'] is not None:
        try:
            return round(float(row['unit_price']), 6)
        except (TypeError, ValueError):
            return 0.0
...
def save_email_config_api():
    ...
    if data.get('unit_price') not in (None, ''):
        try:
            unit_price = round(float(data.get('unit_price')), 6)
        except (TypeError, ValueError):
    ...
    if row:
        ...
        db.execute(
            ...
            "username=?, password=?, from_email=?, from_name=?, unit_price=?, is_active=?, updated_at=? "
            "WHERE id=?",
            (provider, host, port, use_ssl, use_tls, username, new_pwd,
             from_email, from_name, unit_price, is_active, now, row['id'])
        )
    else:
        db.execute(
            "INSERT INTO email_config (provider, host, port, use_ssl, use_tls, username, password, "
            "from_email, from_name, unit_price, is_active, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
            (provider, host, port, use_ssl, use_tls, username,
             password or '', from_email, from_name, unit_price, is_active, now)
        )
    db.commit()
...
@admin_required
def get_email_pricing():
    return jsonify({'unit_price': get_email_unit_price()})


@app.route('/api/config/email/pricing', methods=['POST'])
@admin_required
def set_email_pricing():
    data = request.get_json(silent=True) or {}
    try:
        unit_price = round(float(data.get('unit_price') or 0), 6)
    except (TypeError, ValueError):
    ...
    if row:
        db.execute(
            "UPDATE email_config SET unit_price=?, updated_at=? WHERE id=?",
            (unit_price, now, row['id']))
    else:
        db.execute(
            "INSERT INTO email_config (unit_price, updated_at) VALUES (?, ?)",
            (unit_price, now))
    db.commit()
...
def email_statistics_api():
    ...
    # Billing: a record is billed once (1 email = 1 billing unit) when actually
    # sent (status='sent', NOT simulated). Cost = global unit_price * sent count
    # (facturacion global, no por pais). Simulated/pending/failed/suppressed are
    # excluded so pre-configuration test sends don't inflate the total.
    unit_price = get_email_unit_price()
    sent_count = db.execute(
        f"SELECT COUNT(*) AS n FROM email_records r WHERE {where} AND r.status='sent'",
        params
    ).fetchone()
    sent_n = int(sent_count['n'] or 0)
    total_cost = round(unit_price * sent_n, 6)
    return jsonify({
