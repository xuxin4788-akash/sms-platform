...
- **Contact linking** (`email_replies.contact_id`, nullable FK to contacts, ON DELETE SET NULL; SQLite + PG DDL/migration): inbound replies auto-link a contact by case-insensitive `contacts.email == sender_email`, preferring a contact owned by the same account that sent the original email, else the most recent match. The SPA adds an "Asociar contacto" button in the reply detail — a modal listing the exact-email match or manual name/phone/email search (`GET .../contacts`), then `POST .../contact {contact_id}`; "Quitar vinculo" calls `DELETE .../contact`. Both enforce the reply role scope and the contact visibility scope.
