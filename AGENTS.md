...
| GET/POST | /api/config/email/pricing | Admin | List per-country email unit prices / create one (country unique, case-insensitive → 409 on dup) |
| PUT/DELETE | /api/config/email/pricing/<id> | Admin | Update/delete a per-country email price |
...
### Email billing (facturacion por pais)
- Billing is configured per country in `email_pricing_configs` (id, country UNIQUE case-insensitive, unit_price NUMERIC(14,4) DEFAULT 0, is_active, timestamps), seeded with MX/CO/PE at 0 on new DBs (migration adds the table + seeds if absent on existing DBs). Managed from a standalone `#/email-pricing` page (admin-only, in `AVAILABLE_PAGES`; NOT in `AUTO_GRANT_PAGES`) via `GET/POST /api/config/email/pricing` and `PUT/DELETE /api/config/email/pricing/<id>`.
- Each email is billed **once** (1 email = 1 billing unit, unlike SMS which bills per `billed_segments`). `resolve_email_country_price(country)` returns the unit price for a country (0 if none).
- `GET /api/email/statistics` returns `cost` (total billed) and `by_country` (array of `{country, count, unit_price, cost}`). Only records with `status='sent'` are billed (**simulated/pending/failed/suppressed are excluded**, mirroring SMS billing so pre-configuration test sends don't inflate the total). The country comes from the sending account's `users.country` via a `LEFT JOIN users ON u.id=r.created_by`, joined against the pricing map in Python (`UPPER(TRIM(...))` for case/space normalization, so `MX`/`mx`/` mx ` all resolve).
