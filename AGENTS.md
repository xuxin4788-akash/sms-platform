...
- **`#/email-pricing` is the unified billing page ("Facturacion")**: it renders email (single global `unit_price` via `/api/config/email/pricing`), per-country SMS prices (reads `/api/config/sms`, each `sms_api_configs` row shows its country's `unit_price` inline, saved via `PUT /api/config/sms/<id>` with the full row payload from the cached response), and the SMS global default price (via `GET/PUT /api/settings/billing` `sms_unit_price`). Frontend helpers: `renderEmailPricing`, `saveEmailPricing`, `saveSmsCountryPricing`, `saveSmsDefaultPrice` (all admin-only page).
