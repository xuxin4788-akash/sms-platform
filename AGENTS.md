...
- Per-user fixed extension: the `users.extnumber` column holds an optional fixed SIP extension/phone for an agent. Extensions are **never entered manually**: when `assign_extension=true` is passed on user create/edit (or `assign_extensions=true` on bulk import), the system auto-picks a free one from the configured pool (`voice_extnumber`, comma-separated) and assigns it permanently. If no free extension exists, the operation fails with HTTP 409 and an "ask the system admin to add more extensions" message. An assigned extension can be released via `release_extension=true` (returns to the pool). The agent's own `extnumber` is always used for infin8linx calls; users without one are blocked (HTTP 403) when the provider is real infin8linx (simulation mode still works). The extension actually used is recorded in `voice_records.extnumber`.
