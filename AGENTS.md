...
- **Contact linking** (`email_replies.contact_id`, nullable FK to contacts, ON DELETE SET NULL; SQLite + PG DDL/migration): inbound replies auto-link a contact by case-insensitive `contacts.email == sender_email`, order of preference (1) same APP as the original outbound email AND owned by the account that sent it, (2) same APP, (3) same owner, (4) most recent. **Contact info is shown directly (no manual picker)**: `GET /api/email/replies` auto-resolves a matching contact by sender email even for replies without an explicit `contact_id` (preferring the original APP), returning `contact_name/contact_phone/contact_email/contact_app`. The reply detail renders that contact inline with `Llamar` (tel:) and `WhatsApp` (wa.me) jump buttons. Manual programmatic link/unlink still available via `POST/DELETE /api/email/replies/<id>/contact {contact_id}` and candidate search `GET .../contacts?q=`; both enforce the reply role scope and the contact visibility scope.
