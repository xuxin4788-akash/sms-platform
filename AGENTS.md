...
| GET | /api/email/replies | User | List inbound replies (role scope; search/sender/subject/body; `unread_only`; pagination; rows alias `received_at`/`body_text`; LEFT JOINs the linked contact → `contact_id/contact_name/contact_phone/contact_email/contact_app`) |
| POST | /api/email/replies/&lt;id&gt;/read | User | Mark a reply read (body `{read:true}`), same role scope enforced |
| GET | /api/email/replies/&lt;id&gt;/contacts?q= | User | Contacts to associate: no `q` → visible contacts whose email matches the reply sender; `q` → search name/phone/email (max 50, contact visibility scope) |
| POST/DELETE | /api/email/replies/&lt;id&gt;/contact | User | Link a reply to `{contact_id}` (must be a visible contact, 404 otherwise) / unlink; same reply role scope enforced |
| GET | /api/email/replies/&lt;id&gt;/panel-contact | User | Full inline contact block for a reply: linked contact else auto-resolved by sender email (else `null`), incl. group_name + sms/call/talk stats |
...
- **Contact linking** (`email_replies.contact_id`, nullable FK to contacts, ON DELETE SET NULL; SQLite + PG DDL/migration): inbound replies auto-link a contact by case-insensitive `contacts.email == sender_email`, order of preference (1) same APP as the original outbound email AND owned by the account that sent it, (2) same APP, (3) same owner, (4) most recent. **The reply detail renders the FULL contact block inline (no navigation)**: `GET /api/email/replies/<id>/panel-contact` returns `{contact}` — the reply's linked contact, else auto-resolved by sender email (else `null`) — with `group_name` and activity stats (`sms_count/call_count/talk_time`) via `_contact_stats_map`. The panel shows phone/email/APP/group/remark/amount/discount/payment_link/notes and buttons Llamar (tel:), WhatsApp (wa.me), Zoiper (`dialZoiper`), Editar and Eliminar. Edit (`showEditContactModal`, reuses `/api/contacts?per_page=1000`) and delete (`deleteContact`) accept an optional `onDone` callback so the panel reloads in place (via `loadReplyContactPanel`) instead of navigating to the contacts page. `GET /api/email/replies` still returns lightweight `contact_name/phone/email/app`; manual link/unlink via `POST/DELETE /api/email/replies/<id>/contact {contact_id}` and candidate search `GET .../contacts?q=` (all enforced against reply + contact visibility scope). Note: PG `contacts` has no `updated_at` column, so `_row_with_dates(cd, ('created_at',))` only.
...
### Email billing (facturacion global, no por pais)
- Billing uses a **single global unit price** stored in `email_config.unit_price` (NUMERIC(14,4), default 0; migration adds the column on both new and existing DBs). **Email is not per-country** (unlike SMS, which bills per-country via `sms_api_configs.unit_price`). Configured from the standalone `#/email-pricing` page (admin-only, in `AVAILABLE_PAGES`; NOT in `AUTO_GRANT_PAGES`) via `GET/POST /api/config/email/pricing` (single `unit_price` value; negative rejected, non-numeric → 400).
- **`#/email-pricing` is the unified billing page ("Facturacion")**: it renders email (single global `unit_price` via `/api/config/email/pricing`) and per-country SMS prices maintained in the **standalone `sms_billing_prices` table** (added 2026-09; `country` UNIQUE + `country_name` + `unit_price` + `is_active`, PG/SQLite schema auto-created on startup). SMS prices are added manually via a country dropdown (Mexico/Colombia/Peru/Argentina/Chile) through `GET/POST /api/config/sms/pricing` (POST duplicate country → 409) and `PUT/DELETE /api/config/sms/pricing/<id>`. The **SMS global default price block is NOT rendered** on this page; it is only configured from the SMS API config page (`#/config`, `GET/PUT /api/settings/billing` `sms_unit_price`). Frontend helpers: `renderEmailPricing`, `saveEmailPricing`, `saveSmsCountryPricing`, `addSmsPricing`, `deleteSmsPricing` (all admin-only page). Billing resolution: `get_sms_unit_price_for(uid)` picks `get_country_sms_unit_price(get_effective_sms_country(uid))` → `sms_billing_prices` first, else `sms_api_configs.unit_price`, else the global default; `get_email_unit_price()` reads `email_config.unit_price`.
- Each email is billed **once** (1 email = 1 billing unit, unlike SMS which bills per `billed_segments`). `get_email_unit_price()` returns the current global price (0 if none/unconfigured).
- `GET /api/email/statistics` returns `cost` (total billed) and `unit_price`. Only records with `status='sent'` are billed (**simulated/pending/failed/suppressed are excluded**, mirroring SMS billing so pre-configuration test sends don't inflate the total). Cost = `unit_price * COUNT(* WHERE status='sent')` under the current role scope.
