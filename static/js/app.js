...
async function renderContacts(container) {
    ...
    try {
        ...
        var rows = data.contacts.length === 0
            ? '<tr><td colspan="9" class="text-center text-secondary" style="padding:32px;">No hay contactos</td></tr>'
            : data.contacts.map(function(c) {
                ...
                var linkCell = c.payment_link
                    ...
                    : '<span class="text-secondary text-sm">-</span>';
                return '<tr><td>' + nameCell + '</td><td>' + escapeHtml(c.phone) + '</td><td class="text-secondary text-sm">' + escapeHtml(c.email || '-') + '</td><td>' + (c.group_name ? '<span class="badge badge-blue">' + escapeHtml(c.group_name) + '</span>' : '<span class="text-secondary text-sm">Sin grupo</span>') + '</td><td>' + remarkBadge + '</td><td>' + activityCell + '</td><td>' + amountCell + '</td><td class="text-secondary text-sm">' + escapeHtml(c.notes || '-') + '</td><td>' + linkCell + '</td><td><button class="btn btn-ghost btn-sm btn-icon web-phone-btn" data-phone="' + escapeHtml(String(c.phone).replace(/'/g, '')) + '" onclick="webphoneCall(this, \'' + escapeHtml(String(c.name || '').replace(/'/g, '')) + '\')" title="Llamar (Web)" style="color:#6366F1;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12.01" y2="18"></line></svg></button><button class="btn btn-ghost btn-sm btn-icon" onclick="dialZoiper(\'' + escapeHtml(String(c.phone).replace(/'/g, '')) + '\',\'' + escapeHtml(String(c.name || '').replace(/'/g, '')) + '\')" title="Llamar (suena en Zoiper)" style="color:#10B981;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg></button><button class="btn btn-ghost btn-sm btn-icon" onclick="showEditContactModal(' + c.id + ')" title="Editar"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path></svg></button><button class="btn btn-ghost btn-sm btn-icon" onclick="deleteContact(' + c.id + ')" title="Eliminar" style="color:var(--danger);"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg></button></td></tr>';
            }).join('');
...
async function showEditContactModal(id, onDone) {
    try {
        ...
        var appNames = (appsData.apps || []).map(function(a) { return String(a); });
        var currentApp = contact.app_name || '';
        var hasCurrent = appNames.some(function(a) { return a.toLowerCase() === currentApp.toLowerCase(); });
        var currentOpt = (currentApp && !hasCurrent)
            ? '<option value="' + escapeHtml(currentApp) + '" selected>' + escapeHtml(currentApp) + '</option>'
            : '';
        var appOpts = (currentApp && !hasCurrent ? currentOpt : '') + appNames.map(function(a) {
            var sel = (a.toLowerCase() === currentApp.toLowerCase()) ? ' selected' : '';
...
function showImportModal() {
    api('/api/groups').then(function(data) {
        ...
        var templateBox = '' +
            '<a href="/api/contacts/template-xlsx" download="plantilla_contactos.xlsx" ' +
            ...
            'border-radius:8px;background:#059669;color:#fff;flex-shrink:0;">' +
            '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round">' +
            '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>' +
            '<polyline points="14 2 14 8 20 8"></polyline><line x1="8" y1="13" x2="16" y2="13"></line><line x1="8" y1="17" x2="16" y2="17"></line></svg>' +
            '</span>' +
            '<span style="flex:1;line-height:1.3;">' +
            '<span style="display:block;font-size:14px;font-weight:600;color:#1E293B;">Descargar plantilla Excel (.xlsx)</span>' +
            '<span style="display:block;font-size:12px;color:#64748B;margin-top:2px;">Las columnas Grupo, Nota y APP tienen lista desplegable</span>' +
            '</span>' +
            ...
            '<span style="display:block;font-size:12px;color:#64748B;margin-top:2px;">Columnas: name, phone, email, notes, remark, app_name, amount, discount_amount, payment_link, group</span>' +
            '</span>' +
...
// ============================================================
// Web phone (WebRTC softphone) — click-to-call from Contacts
// ============================================================
// The SPA embeds /static/phone/ in an off-screen iframe. That page
// auto-registers the current extension (same-origin localStorage) and we
...
function checkCharsetInfo(content) {
    ...
    infoEl.innerHTML = 'Tarifa: <strong>' + data.lang + ' (' + (data.billing_class === 'latin' ? '160' : '70') + '/SMS)</strong> | Caracteres: ' + data.char_count + '/' + data.single + ' | Partes: <strong>' + data.parts + '</strong> SMS';
}
...
function toggleApiConfig() {
    ...
    var isTeamAdmin = (roleSelect.value === 'team_admin');
    // Pais: obligatorio para el Administrador de Equipo (todas las cuentas del
    // equipo siguen su pais para la API SMS/voz); opcional/heredado para miembros.
    ...
    if (countrySelect) {
        ...
        if (isTeamAdmin) {
            if (!countrySelect.value) {
                // Insert a mandatory placeholder so the user must choose.
                var ph = document.createElement('option');
                ph.value = ''; ph.textContent = 'Seleccionar pais...'; ph.disabled = true; ph.selected = true;
                countrySelect.insertBefore(ph, countrySelect.firstChild);
            } else if (countrySelect.options[0] && countrySelect.options[0].textContent === 'Sin pais especifico') {
                // keep normal flow
            }
        }
    ...
    if (equipoGroup) {
        equipoGroup.style.display = isTeamAdmin ? 'none' : '';
    }
}
...
async function showEditUserModal(id) {
    ...
    if (u.role === 'team_admin') {
        // El Administrador de Equigo DEBE tener pais: todas las cuentas del equipo
        // siguen su pais para la API SMS/voz. Si aun no tiene, se exige elegir.
...
async function handleBulkCreate(event) {
    ...
    // Al crear Administradores de Equipo el pais es obligatorio (default o por linea).
    if (state.user.role === 'admin' && !defaultCountry) {
        showToast('Selecciona un pais por defecto para los Administradores de Equipo (o indiquelo por linea)', 'error');
        return;
    }
...
async function handleBulkImport(event) {
    ...
    // Al importar Administradores de Equipo el pais es obligatorio (default o por fila).
    if (state.user.role === 'admin' && (!importCountry || !importCountry.value)) {
        showToast('Selecciona un pais por defecto para los Administradores de Equipo (o indiquelo por fila)', 'error');
        return;
    }
...
async function renderMyAccount(container) {
    ...
    try {
        ...
        var myCost = myAcct.total_cost != null ? Number(myAcct.total_cost) : (Number(myAcct.total || 0) * Number(data.unit_price || 0));
        var billingNote = Number(data.unit_price) > 0 ? ('Costo por SMS facturado: ' + formatPrice(data.unit_price)) : 'Costo por SMS no configurado';
        var html = filterHtml + '<div class="card mb-4"><div class="card-header card-header-wrap"><span style="font-size:20px;"></span><h3 style="margin:0;">Resumen de Cuenta</h3><span class="badge badge-primary header-badge">' + escapeHtml(state.user.username) + '</span></div><div class="card-body"><div class="stats-grid" style="grid-template-columns:repeat(auto-fit,minmax(140px,1fr));">' + statCard('Total SMS', myAcct.total || 0) + statCard('SMS Facturados', myBilled, '#0F766E') + statCard('Enviados', myAcct.sent || 0, 'var(--success)') + statCard('Fallidos', myAcct.failed || 0, 'var(--danger)') + statCard('Costo Total', formatCost(myCost), 'var(--primary)') + statCard('Tasa de Exito', myRate + '%') + '</div><table class="info-table"><tbody><tr><td class="info-label">SMS Pendientes</td><td class="info-value">' + (myAcct.pending || 0) + '</td></tr><tr><td class="info-label">Facturacion</td><td class="info-value">' + billingNote + ' <span class="text-secondary">(se cobra por segmento: 70 car. espanol/chino, 160 ingles/indonesio; exitoso o fallido)</span></td></tr></tbody></table></div></div>';
    ...
    try {
        ...
        if (allTeamsList && allTeamsList.length > 0) {
            var teamRows = allTeamsList.map(function(t) {
                ...
                var costVal = t.total_cost != null
                    ? Number(t.total_cost)
                    : (Number(t.total) || 0) * Number(data.unit_price || 0);
                return '<tr><td>' + nameCell + '</td><td style="text-align:center;">' + t.member_count + '</td><td style="text-align:right;font-weight:600;">' + t.total + '</td><td style="text-align:right;font-weight:600;color:#0F766E;" title="SMS facturados por segmento (70 car. espanol/chino, 160 ingles/indonesio)">' + (t.billed_segments != null ? t.billed_segments : t.total) + '</td><td style="text-align:right;color:var(--success);">' + t.sent + '</td><td style="text-align:right;color:var(--danger);">' + t.failed + '</td><td style="text-align:right;" title="Precio medio por SMS facturado (por pais)">' + formatPrice(Number(t.unit_price) || 0) + '</td><td style="text-align:right;color:var(--primary);font-weight:600;">' + formatCost(costVal) + '</td><td style="text-align:right;font-weight:600;" title="Correos realmente enviados (excluye simulados, pendientes, fallidos)">' + (t.email_sent || 0) + '</td><td style="text-align:right;color:var(--primary);" title="Costo de correos = precio global por correo x enviados">' + formatCost(Number(t.email_cost) || 0) + '</td><td style="text-align:right;color:var(--primary);">' + (t.today || 0) + '</td><td style="text-align:right;"><span class="badge ' + rateClass + '">' + r + '%</span></td></tr>';
            }).join('');
            html += '<div class="card mb-4"><div class="card-header card-header-wrap"><h3 style="margin:0;">Equipos</h3><span class="badge header-badge" style="background:var(--primary);color:#fff;">' + allTeamsList.length + ' equipos</span></div><div class="card-body"><div class="table-container"><table><thead><tr><th>Equipo</th><th style="text-align:center;">Miembros</th><th style="text-align:right;">Total SMS</th><th style="text-align:right;" title="SMS facturados por segmento (70 car. espanol/chino, 160 ingles/indonesio)">SMS Fact.</th><th style="text-align:right;">Enviados</th><th style="text-align:right;">Fallidos</th><th style="text-align:right;" title="Precio medio por SMS facturado (por pais)">Precio</th><th style="text-align:right;">Costo</th><th style="text-align:right;" title="Correos realmente enviados (excluye simulados, pendientes, fallidos)">Correos</th><th style="text-align:right;" title="Costo de correos (precio global x enviados)">Costo Correo</th><th style="text-align:right;">Hoy</th><th style="text-align:right;">Exito</th></tr></thead><tbody>' + teamRows + '</tbody></table></div></div></div>';
        } else {
...
async function handleSaveApiConfig(event, configId) {
    ...
    var name = card.querySelector('h3').textContent.trim().split(' ')[0];
    var price = parseFloat(form.unit_price.value);
    if (isNaN(price) || price < 0) { showToast('Ingrese un costo por SMS valido', 'error'); return; }
...
function updateEmbedCounter() {
    var ta = document.getElementById('quick-content');
    if (!ta) return;
    var data = localSmsParts(ta.value);
    var cc = document.getElementById('embed-char-count');
    var sc = document.getElementById('embed-sms-count');
    if (cc) cc.textContent = data.char_count;
    if (sc) sc.textContent = data.parts;
}
...
// Tab navigation for the unified config page. Switching only toggles the
// visible body (opacity/display), keeping already-rendered DOM alive.
function switchUnifiedTab(key) {
    ['apis', 'email', 'senders'].forEach(function(k) {
        var btn = document.getElementById('utab-' + k);
        if (btn) btn.classList.toggle('active', k === key);
        var body = document.getElementById('utab-body-' + k);
        if (body) body.style.display = (k === key) ? '' : 'none';
    });
    window._unifiedTab = key;
}

// Render the "Envios por APP" block inside a unified-config tab (no page
// chrome / standalone heading since it lives inside the tab area).
async function renderEmailSendersTab(container) {
    container.innerHTML = '<div class="text-center text-secondary">Cargando...</div>';
    try {
...
async function renderEmailPricing(container) {
    ...
    try {
        ...
        var opts = SMS_PRICING_COUNTRIES.filter(function(c) { return !existingCountries[c.code]; });
        var ddOptions = opts.length
            ? opts.map(function(c) { return '<option value="' + c.code + '">' + escapeHtml(c.name) + '</option>'; }).join('')
        ...
        var smsRows = smsConfigs.length ? smsConfigs.map(function(c) {
            return '<tr>' +
                ...
                '<td class="text-right" style="white-space:nowrap;">' +
                  '<button class="btn btn-primary btn-sm" onclick="saveSmsCountryPricing(' + c.id + ')">Guardar</button> ' +
        ...
        container.innerHTML =
            ...
            '<div class="card mb-4"><div class="card-header"><h2 style="margin:0;">SMS (facturacion por pais)</h2></div><div class="card-body">' +
                '<p class="text-secondary" style="margin-bottom:14px;">El costo por SMS se factura por <strong>segmento</strong> y usa el <strong>precio configurado para cada pais</strong> (pais de cada cuenta). Agregue manualmente el precio por pais.</p>' +
                '<div class="form-row" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;align-items:end;margin-bottom:16px;">' +
                  '<div class="form-group"><label>Pais</label><select id="sp-country" style="max-width:220px;">' + ddOptions + '</select></div>' +
                  '<div class="form-group"><label>Precio por SMS</label><input id="sp-new-price" type="number" step="any" min="0" value="0" style="max-width:220px;"></div>' +
                  '<div><button class="btn btn-success" onclick="addSmsPricing()">Agregar</button></div>' +
                '</div>' +
...
async function addSmsPricing() {
    var msg = document.getElementById('sp-msg');
    if (msg) msg.innerHTML = '';
    var country = document.getElementById('sp-country');
    var countryName = '';
    var priceInput = document.getElementById('sp-new-price');
    if (!country || !country.value) { if (msg) msg.innerHTML = '<div class="alert alert-error">Seleccione un pais.</div>'; return; }
    for (var i = 0; i < SMS_PRICING_COUNTRIES.length; i++) {
        if (SMS_PRICING_COUNTRIES[i].code === country.value) { countryName = SMS_PRICING_COUNTRIES[i].name; break; }
    }
    var price = parseFloat(priceInput.value);
    if (isNaN(price) || price < 0) { if (msg) msg.innerHTML = '<div class="alert alert-error">Ingrese un precio valido.</div>'; return; }
    try {
        await api('/api/config/sms/pricing', { method: 'POST', body: { country: country.value, country_name: countryName, unit_price: price } });
        if (msg) msg.innerHTML = '<div class="alert alert-success">Precio por SMS (' + escapeHtml(countryName) + ') agregado.</div>';
        renderEmailPricing(document.getElementById('page-content'));
    } catch (e) { if (msg) msg.innerHTML = '<div class="alert alert-error">' + escapeHtml(e.message) + '</div>'; }
}
...
async function saveSmsCountryPricing(configId) {
    ...
    try {
        await api('/api/config/sms/pricing/' + configId, { method: 'PUT', body: { unit_price: price } });
        if (msg) msg.innerHTML = '<div class="alert alert-success">Precio por SMS actualizado.</div>';
        renderEmailPricing(document.getElementById('page-content'));
    } catch (e) { if (msg) msg.innerHTML = '<div class="alert alert-error">' + escapeHtml(e.message) + '</div>'; }
}

async function deleteSmsPricing(configId) {
    var msg = document.getElementById('sp-msg');
    if (msg) msg.innerHTML = '';
    if (!confirm('Seguro que desea eliminar el precio por SMS de este pais?')) return;
    try {
